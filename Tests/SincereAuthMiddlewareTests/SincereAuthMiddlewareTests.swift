...
import XCTVapor
import JWTKit
@testable import SincereAuthMiddleware

final class SincereAuthMiddlewareTests: XCTestCase {
  ...
  override func setUpWithError() throws {
    ...
    app.routes.group(SincereAuthMiddleware()) { secure in
      secure.get("hello") { req -> String in
        let token = req.sincereAuthSessionToken!
        return token.userID?.uuidString ?? "not a uuid"
      }
    }

    app.routes.group(SincereAuthMiddleware(requiredRole: "admin")) { secure in
    ...
    let token = SincereAuthSessionToken(userID: userID,
                                   deviceName: "Xample",
                                   roles: [],
  ...
  func testAuthorizedTokenIsValid() throws {
    let decoded = try app.jwt.signers.verify(authorizedToken, as: SincereAuthSessionToken.self)
